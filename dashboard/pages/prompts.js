async function renderPrompts() {
  ...
  try {
    ...
    if (entries.length === 0) {
      grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><div class="empty-state-icon">${icon('file-text', 32)}</div><div class="empty-state-title">No prompt templates</div></div>`;
      return;
    }
    ...
    grid.innerHTML = entries.map(([name, content]) => {
      ...
      return `<div class="skill-card" data-act="viewPrompt" data-arg="${escapeHtml(name)}">
        <div class="skill-card-header">
          <div class="skill-card-icon">${icon('file-text', 16)}</div>
    ...
    document.getElementById('promptGrid').innerHTML = `<div class="empty-state" style="grid-column:1/-1"><div class="empty-state-icon">${icon('alert', 32)}</div><div class="empty-state-title">${escapeHtml(err.message)}</div></div>`;
  }
}
...
async function viewPrompt(name) {
  ...
  `, `
    <button class="btn btn-primary" onclick="copyPromptFromModal()">${icon('clipboard', 13)} Copy</button>
