  ...
  content.innerHTML = `
    <div class="page-header">
      ...
      <div class="btn-group">
        ...
        <button class="btn btn-ghost" onclick="renderJournal()">${icon('refresh', 13)} Refresh</button>
      </div>
    </div>
    ...
    <div class="flex gap-3" style="margin-bottom:16px">
      <div style="flex:1;display:flex;gap:8px">
        ...
        <button class="btn btn-ghost" onclick="searchJournal()">${icon('search', 13)} Search</button>
      </div>
    </div>
...
async function loadJournalEntries() {
  try {
    ...
    if (!list) return;
    if (entries.length === 0) {
      list.innerHTML = `<div class="empty-state"><div class="empty-state-icon">${icon('book', 32)}</div><div class="empty-state-title">No journal entries yet</div><div class="empty-state-desc">Write your first entry above</div></div>`;
      return;
    }
    list.innerHTML = `
...
async function searchJournal() {
  ...
  try {
    ...
    if (!list) return;
    if (results.length === 0) {
      list.innerHTML = `<div class="empty-state"><div class="empty-state-icon">${icon('search', 32)}</div><div class="empty-state-title">No results for "${escapeHtml(q)}"</div></div>`;
      return;
    }
    list.innerHTML = `
